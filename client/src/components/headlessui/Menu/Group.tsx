...
const Group: React.FC<Props> = (props) => {
  ...
  if (drawer) {
    ...
    if (!hasChildren) {
      groupSelectClass += 'hover:!bg-accent hover:!text-accent-foreground ';
      if (isActive) {
        groupSelectClass +=
    ...
    if (isActive) {
      groupSelectClass += 'text-accent-foreground bg-accent hover:!bg-accent ';
    }
  }
  ...
  return (
    <div>
      <ul className="group">
        {drawer && <div className="my-3 pl-6 text-xs">{item.group}</div>}
        {/*  */}
        <div
          className={` ${groupSelectClass} group-has-[.selected]:text-primary group-has-[.invisible]:hover:bg-accent`}
        >
          <a className="my-1 block" onClick={() => handleGroupClick(item)}>
            <div
            ...
            >
              ...
              {drawer && <div className="my-1 flex-1">{item.title}</div>}
              {drawer && hasChildren && (
                <div className="w-3">
                  {open ? (
